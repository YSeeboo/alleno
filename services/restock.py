...
def list_pending_summary(db: Session) -> list[dict]:
    ...
    # qty per source comes from the user-entered shortfall_qty (差额) on
    # the restock record itself — that's the single source of truth for
    # "how much to restock", not the handcraft order's qty.
    ...
    for r in rows:
        ...
        bucket["sources"].append({
            ...
            "qty": float(r.shortfall_qty) if r.shortfall_qty is not None else None,
        })
